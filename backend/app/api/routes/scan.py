...
async def scan_image(
    ...
    # Debug logs for language + text + Kazakh engine status
    logger.warning("SCAN LANG = %s", lang)
    logger.warning("TEXT TO SPEAK = %s", result.text)
    logger.warning("TTS ENGINE KZ LOADED = %s", tts_engine._kz_engine is not None)
    logger.warning("PIPELINE DETECTION DICTS = %s", result.detection_dicts)
